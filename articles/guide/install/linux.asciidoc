...
These instructions are for Debian-based and RPM-based distributions.
...
On Fedora (since version 18) and RHEL (since version 8), you use the DNF package manager.
...
. Ensure that you are running Java version 11 by running:
+
----
java -version
----
+
If you are not using version 11, change it with:
+
----
sudo alternatives --config java
----
+
Select OpenJDK version 11.
...
Depending on your system version, you may need to install package repositories or perform some other steps.
...
If you want to install Amazon Corretto, rather than OpenJDK, you either have to add the package repository or install it from a `.deb`, `.rpm` or `.tar` package.

See:

* Instructions for installing from an APT (Debian) or YUM (RPM) repository in https://docs.aws.amazon.com/corretto/latest/corretto-11-ug/linux-info.html[Amazon Corretto 11 Guide for Linux]
