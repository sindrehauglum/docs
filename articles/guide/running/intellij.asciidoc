...
If you are developing a Spring Boot project, it is easier to run a Java web application, because Spring Boot takes care of starting and configuring the server.

To run your application, all you need to do is run the [classname]#Application# class that contains the [methodname]#main()# method that starts Spring Boot.
IntelliJ automatically detects that you have such a a class and displays it in the run configurations dropdown.

To start the application, either:

* Click the "play" button next to the run configurations dropdown.
* Open the `Application.java` file and click the "play" button next to the code line containing the main method.
...
. Open the *Maven* view by clicking the vertical tab at the right-hand side of the IntelliJ IDEA window:
+
.Maven projects view
...
Finally, at the `install` phase, it downloads dependencies and copies the generated artifacts into your local Maven repository as part of the process.
...
You are likely to want to use a goal to run the application frequently during  development, so you may want to create a _running configuration_ for it.

A _running configuration_ is a _shortcut_ to run a specific task from within the IDE.
Here, we create a running configuration for the `jetty:run` Maven goal to make it simpler to run the web application.
...
You should see the new option at the top-right corner of IntelliJ IDEA:
...
If you edit and save any of the source files, they will be compiled automatically, but you need to restart the server in order to see the changes.
