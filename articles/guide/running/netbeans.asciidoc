...
While the server is running, you can edit and save any source files. The server notices the change and redeploys the web application.
...
. In *Execute Goals*, type the goal to start the web server: `jetty:run` (plain Java project), `tomee:run` (CDI project), or `spring-boot:run` (Spring Boot project).
. Click [guibutton]#OK#:
+
...
If the Vaadin application was created with the project wizard or a Maven archetype, it should be available at http://localhost:8080/[localhost:8080].

Using the above configuration, you can now use NetBeans to develop your Vaadin application with Maven.
